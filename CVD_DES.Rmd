...
inputs = list(
  ...
  mortalityAfterASCVD_low = 1.60,
  mortalityAfterASCVD_upp = 2.40, 
  mortalityAfterASCVD_dist = "lnorm", # log-normal
...
```{r gompertz-hr}
# # male
# mlt2020_hr = data.frame(Age = 0:(nrow(mlt2020_nonCVD)-1), qx = sapply(sapply(mlt2020_nonCVD$qx, ProbToRate, t = 1) * inputs$mortalityAfterASCVD, RateToProb, t = 1), lx = rep(0,nrow(mlt2020_nonCVD)), dx = rep(NA,nrow(mlt2020_nonCVD)))
# 
...
# # female
# 
# flt2020_hr = data.frame(Age = 0:(nrow(flt2020_nonCVD)-1), qx = sapply(sapply(flt2020_nonCVD$qx, ProbToRate, t = 1) * inputs$mortalityAfterASCVD, RateToProb, t = 1), lx = rep(0,nrow(flt2020_nonCVD)), dx = rep(NA,nrow(flt2020_nonCVD)))
# 
...
trace %>% mutate(cost = basic_CVD_costs(inputs, death_after_ASCVD, death_of_ASCVD, death_without_CVD, get_CVD, time_in_model, initial_age),
  ...
  kable(caption = "trace of simulated individuals with cost and utilities") %>%
  kable_styling() %>%
  scroll_box(height = "400px")
