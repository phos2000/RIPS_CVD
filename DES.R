...
assign_events <- function(traj, inputs)
{
  ...
  {
    traj <<- log_(traj, paste0("assign ", event$name, "/", event$attr), level=1)
...
process_events <- function(traj, env, inputs)
{
  ...
  traj <- timeout(traj, function()
  {
    ...
    log_(traj, paste0(" Next up => ",event$name,"\n"), level=1)
    log_(traj, paste0("            waiting", event_time-now(env), '\n'), level=1)
  ...
  # Age them by clock
  #cat("age by clock\n")
#  traj <- set_attribute(traj,'age',function() {
#    get_attribute(env,"ageInitial")+(now(env)/365.0)
#  })
  #cat("out of age by clock\n")
  ...
  args <- lapply(event_registry,FUN=function(e) {
    #log_(traj, paste0(e$name, "/", e$attr), level=1)
    trajectory(e$name) %>%
      e$func(inputs)   %>%
